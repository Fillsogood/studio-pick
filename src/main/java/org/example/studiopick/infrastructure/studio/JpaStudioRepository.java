...
public interface JpaStudioRepository extends JpaRepository<Studio, Long>, JpaSpecificationExecutor<Studio> {
  ...
  @Query("SELECT s FROM Studio s WHERE s.status = 'ACTIVE' AND (:region IS NULL OR SUBSTRING(s.location, 1, 2) = :region) AND (:keyword IS NULL OR s.name LIKE %:keyword% OR s.description LIKE %:keyword%)")
