...
public class ReportServiceImpl implements ReportService {
    ...
    public void processReport(Long reportId, Long adminId, ReportStatus status, String adminComment) {
        ...
        switch (status) {
            case REVIEWED -> report.markAsReviewed(admin, adminComment);
            case RESTORED -> {
            ...
            default -> throw new IllegalArgumentException("유효하지 않은 신고 처리 상태입니다.");
        }
    ...
    public boolean autoHideContent(ReportType reportType, Long reportedId) {
        try {
            return switch (reportType) {
                case ARTWORK -> hideArtwork(reportedId);
                case CLASS -> hideWorkShop(reportedId);
                case REVIEW -> hideStudio(reportedId);
                default -> {
    ...
    private boolean hideWorkShop(Long workShopId) {
        return jpaWorkShopRepository.findById(workShopId)
                .map(workShop -> {
                    ...
                    log.info("WorkShop {} auto-deactivated due to reports", workShopId);
                    return true;
                }).orElse(false);
    }
        ...
        return jpaStudioRepository.findById(studioId)
                .map(studio -> {
                    if (studio.getHideStatus() == HideStatus.REPORTED) {
                        ...
                        jpaStudioRepository.save(studio);
                        log.info("Studio {} restored", studioId);
                        return true;
                    }
                    return false;
                }).orElse(false);
